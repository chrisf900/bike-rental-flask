...
def get_paginated_user_trips_by_user(
    user_uuid: UUID | str, page: int = 1, per_page: int = 5, max_per_page: int = 5
) -> Query:
    """
    Get paginated user trips
    :param user_uuid: UUID | str
    :param page: int = 1
    :param per_page: int = 10
    :param max_per_page: int = 5
    :return: Query
    """
    query = providers.get_trips_by_user_uuid(user_uuid=user_uuid)
...
def get_country_by_code(country_code: str) -> Query:
    return providers.get_country_by_code(country_code=country_code)
...
def start_trip(
    ...
    current_trip = providers.get_current_user_trip(user_uuid=user_uuid)
